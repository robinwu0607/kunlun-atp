...
``主机名称.py`` 文件的内容示例如下::
    ...
    GUI_CONFIG = [
        dict(name="FT", desc="仅用于演示", uut=8, sequencer="project.main", params=dict(ip="192.168.0.1"))
    ]
...
5. ``params`` 定义此工站的硬件参数，用户代码中使用 ``lib.get_params()`` 调用。

其中，以上示例中的 ``sequencer`` 把 **FT工站** 与 ``project.main`` 关联起来了，因此 **FT工站** 下执行
